...
#slider_u0 = tkinter.DoubleVar(0.1)
pi = 3.1415926535
oncanvas = False
update = True
nheight = 100
...
u2 = zeros((int(height), int(width)))
#u21 = zeros((int(height/4), int(width/4)))
# 初始化
...
def curl(ux, uy):
    global ncurl
    ncurl = roll(uy, -1, axis=1) - roll(uy, 1, axis=1) - roll(ux, -1, axis=0) + roll(ux, 1,
                                                                                                             axis=0)
    return ncurl
        ...
        for j_write in range(width):
            ...
            for i_str in range(len(ostring)):
                if ostring[i_str] == 'e':
                    ...
                    ostr = str(float(ostr) * power(10, es))
            output.write(ostr)
            '''
            if j_write != width - 1:
                output.write(',')
...
class Main_window(tkinter.Frame):
    def __init__(self, master):
        ...
        self.menu_operation.add_command(label='动画加速',command=None)
        self.menu_operation.add_command(label='动画减速',command=None)
        self.menu_operation.add_separator()
        self.menu_operation.add_command(label='增加对比度',command=None)
        self.menu_operation.add_command(label='减小对比度',command=None)
        """
        self.menu_operation.add_separator()
        ...
        #canvaseq.get_tk_widget().bind("<Button-1>", draw_start)
        #canvaseq.get_tk_widget().bind("<ButtonRelease-1>", draw_stop)
        ...
        #self.frame_fluid = tkinter.Frame(root, width=300, height=300, relief = 'groove', bd=1)
        self.frame_fluid.grid(column=0, row = 0)

        self.frame_barrier = tkinter.LabelFrame(self.layout_snf, width=300, height=100, text="障碍参数", bd=1)
        self.frame_barrier.grid(column=0, row = 1)

        self.frame_animation = tkinter.LabelFrame(self.layout_snf, width=300, height=100, text="动画参数", bd=1)
        self.frame_animation.grid(column=0, row = 2)
        ...
        #self.button_pause.grid(column=0, row=13)
        ...
        self.button_equation = ttk.Button(self.frame_barrier, text='高级障碍', command=self.Equation_window)
        ...
        col_count, row_count = self.layout_snf.grid_size()
        for col in range(col_count):
            self.layout_snf.grid_columnconfigure(col, minsize=75)
        for row in range(row_count):
            self.layout_snf.grid_rowconfigure(row, minsize=75)

        col_count, row_count = self.layout_window.grid_size()
        for col in range(col_count):
            self.layout_window.grid_columnconfigure(col, minsize=100)
        for row in range(row_count):
            self.layout_window.grid_rowconfigure(row, minsize=100)
    ...
    def Equation_window(self):
        global canvaseq, ProgressbarEq

        equation_window = tkinter.Toplevel(root)
        #equation_window.geometry('1080x400')
        equation_window.resizable(width=False, height=False)
        ...
        self.frame_eq_t = tkinter.Frame(equation_window)
        self.frame_eq_t.grid(column=0, row = 2, columnspan=2)

        Labal_x = tkinter.Label(equation_window, text="x(t) =")
        Labal_x.grid(row=0, column=0)

        Labal_y = tkinter.Label(equation_window, text="y(t) =")
        ...
        Entry_x = tkinter.Entry(equation_window, width=65, textvariable=in_xt)
        Entry_x.grid(row=0, column=1)

        in_yt = tkinter.StringVar()
        in_yt.set('10*cos(t);')
        Entry_y = tkinter.Entry(equation_window, width=65, textvariable=in_yt)
        ...
        canvaseq = FigureCanvasTkAgg(eqFig, equation_window)
        #canvaseq.get_tk_widget().bind("<Button-1>", draw_start)
        #canvaseq.get_tk_widget().bind("<ButtonRelease-1>", draw_stop)
        canvaseq.get_tk_widget().grid(row=3, column=0, columnspan=2)

        EqButton_Confirm = ttk.Button(equation_window, text='确定',
        ...
        ProgressbarEq = ttk.Progressbar(equation_window, length=80, maximum=100)
        ProgressbarEq.grid(row=1, column=3)

        self.slider_eq_x = ttk.Scale(equation_window, from_=-int(width/2), to=int(width/2), orient=tkinter.HORIZONTAL,
                                          command=self.equation_window.Change_x, length=500)
        self.slider_eq_x.set(0)
        self.slider_eq_x.grid(column=1, row=5, columnspan=2)

        self.slider_eq_y = ttk.Scale(equation_window, from_=-int(height/2), to=int(height/2), orient=tkinter.VERTICAL,
                                          command=self.equation_window.Change_y, length=300)
        self.slider_eq_y.set(0)
        self.slider_eq_y.grid(column=3, row=3)

        self.label_dx = tkinter.Label(equation_window, textvariable=self.text_dx)
        self.label_dx.grid(column=0, row=5)

        self.label_dy = tkinter.Label(equation_window, textvariable=self.text_dy)
        self.label_dy.grid(column=3, row=2)

        """ 老版本UI"""
        """
        Labal_x = tkinter.Label(equation_window, text="x(t)=")
        Labal_x.grid(row=1, column=1)

        Labal_y = tkinter.Label(equation_window, text="y(t)=")
        Labal_y.grid(row=2, column=1)

        Labal_t = tkinter.Label(equation_window, text="t的起始值:")
        Labal_t.grid(row=3, column=1)
        Labal_te = tkinter.Label(equation_window, text="t的终止值:")
        Labal_te.grid(row=4, column=1)

        Labal_ex = tkinter.Label(equation_window, text="计算精度：")
        Labal_ex.grid(row=5, column=1)

        in_xt = tkinter.StringVar()
        in_xt.set('10*sin(t);')  # 预输入的值
        Entry_x = tkinter.Entry(equation_window, width=40, textvariable=in_xt)
        Entry_x.grid(row=1, column=2)

        in_yt = tkinter.StringVar()
        in_yt.set('10*cos(t);')
        Entry_y = tkinter.Entry(equation_window, width=40, textvariable=in_yt)
        Entry_y.grid(row=2, column=2)

        in_t = tkinter.StringVar()
        in_t.set('-200')
        Entry_t = tkinter.Entry(equation_window, width=10, textvariable=in_t)
        Entry_t.grid(row=3, column=2)
        in_te = tkinter.StringVar()
        in_te.set('200')
        Entry_te = tkinter.Entry(equation_window, width=10, textvariable=in_te)
        Entry_te.grid(row=4, column=2)

        in_ex = tkinter.StringVar()  # 计算精度
        in_ex.set('2')
        Entry_ex = tkinter.Entry(equation_window, width=10, textvariable=in_ex)
        Entry_ex.grid(row=5, column=2)

        canvaseq = FigureCanvasTkAgg(eqFig, equation_window)
        canvaseq.get_tk_widget().bind("<Button-1>", draw_start)
        canvaseq.get_tk_widget().bind("<ButtonRelease-1>", draw_stop)
        canvaseq.get_tk_widget().place(x=520, y=10)

        nnbarrier = nbarrier
        #fresheq()
        # canvaseq.bind("<Button-1>", lambda: thread_it(draw_start))
        # canvaseq.bind("<ButtonRelease-1>", draw_stop)

        text_eq_eg = tkinter.Text(equation_window, width=40, height=15)
        text_eq_eg.insert(tkinter.END, "在上面输入曲线的参数方程。输入完毕后单击确定，在左侧观测和图像，如果正确，请单击“确认”。\n"
                                       "如果想要输入多个参数方程，请用分号隔开并一一对应\n"
                                       "例如:\n\n"
                                       ""
                                       "(1)半径为10px的圆:\n"
                                       "x(t) = 10*sin(t);\n"
                                       ...
                                       ""
                                       "P.S.计算精度建议小于10")
        text_eq_eg["state"] = tkinter.DISABLED
        text_eq_eg.place(x=2, y=125)

        EqButton_Confirm = tkinter.Button(equation_window, text='确定', width=5, height=1,
                                          command=lambda: thread_it(Equation, Entry_x.get(), Entry_y.get(),
                                                                    Entry_t.get(),
                                                                    Entry_te.get(), Entry_ex.get()))
        EqButton_Confirm.grid(row=2, column=3)
        ProgressbarEq = ttk.Progressbar(equation_window, length=100, maximum=100)
        ProgressbarEq.place(x=400, y=25)

        Button_Confirm = tkinter.Button(equation_window, text='确认', width=5, height=1,
                                        command=lambda: Equation(1, 1, 1, 1, 1))
        Button_Confirm.place(x=800, y=310)
        ProgressbarConfirm = ttk.Progressbar(equation_window, length=100, maximum=100)
        ProgressbarConfirm.place(x=850, y=315)

        Button_rot = tkinter.Button(equation_window, text='旋转', width=5, height=1,
                                    command=lambda: thread_it(HelpRot, pi / 12))
        Button_rot.place(x=800, y=350)

        CheckDraw = tkinter.Checkbutton(equation_window, text='绘制', variable=drawing_start, onvalue=True,
                                        offvalue=False)
        CheckDraw.place(x=850, y=350)
        """
    ...
    def LiftDragWindow(self):
        ...
        #canvaseq.get_tk_widget().bind("<Button-1>", draw_start)
        #canvaseq.get_tk_widget().bind("<ButtonRelease-1>", draw_stop)
        canvasld.get_tk_widget().grid(column=0, row=0)
    ...
    def Reset(self):
        ...
        #self.label_status_fps.setStyleSheet("color:black")
    ...
    def fresh_status(self):
        ...
        self.text_label_status_fps.set("帧率：" + str(fps) + " 帧/秒        ")
        self.text_label_status_gps.set("代率：" + str(gps) + " 代/秒        ")
        #self.label_status_barrier.setHidden(True)
        '''
        if self.checkbox_changeable.isChecked():
            self.label_status_barrier.setText("障碍：可变")
        else:
            self.label_status_barrier.setText("障碍：不可变")
        '''
        if alert:
    ...
    def FreshDetail(self):
        try:
            if xdata != None:
                ...
                #self.label_status_barrier.setHidden(False)
        except:
            traceback.print_exc()
            #self.fresh_status()
...
class Equation_window():
    ...
    def Change_x(self, *arg):
        ...
        nnbarrier = roll(nnbarrier, int(self.slider_x.value() - self.last_x), axis=1)
    ...
    def Change_y(self, *arg):
        ...
        nnbarrier = roll(nnbarrier, int(self.slider_y.value() - self.last_y), axis=0)
